...
class Discord_Info(commands.Cog):
    ...
    async def avatar(self, ctx, member: discord.Member = None):
        if not member:
            av = discord.Embed(color=discord.Color.blue(), title="This is how you look like",
                               timestamp=datetime.datetime.utcfromtimestamp(1600786335))
            av.set_author(name=str(ctx.author)[:-5], icon_url=ctx.author.avatar_url, )
            av.set_image(url=ctx.author.avatar_url)
            await ctx.send(embed=av)
        else:
            av = discord.Embed(color=discord.Color.blue(), title=f"This is how {str(member)[:-5]} looks like",
                               timestamp=datetime.datetime.utcfromtimestamp(1600786335))
            av.set_author(name=str(ctx.author), icon_url=ctx.author.avatar_url)
    ...
    @server.command(name="server avatar", aliases=["icon"])
